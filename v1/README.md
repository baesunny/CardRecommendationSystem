...
현재 v1에서는 100종 전체 카드 혜택을 미리 파싱해 `outputs/card_parsed_benefits.json`에 저장하고, 추천 시에는 이 파일에서 즉시 조회한다.

| 구분 | 실시간 파싱 (초기) | 사전 파싱 (현재 v1) |
|------|-------------------|---------------------|
...
- **초기 방식의 문제**: GPT 한 번 호출로 파싱·계산·설명을 모두 처리 → 산수 오류 빈번
- **역할 분리**: LLM(파싱) → Python(계산) → LLM(설명)
- **사전 파싱**: 100종 카드 혜택 오프라인 파싱으로 추천 응답 속도 대폭 단축
